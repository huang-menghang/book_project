                    ...
                    <select name="status" class="form-control inline">
                        ...
                        <?php foreach ($status_mapping as $_status => $_title): ?>
                            <option value="<?= $_status; ?>"
                                    <?php if ($_status == $search_conditions['status']): ?>selected<?php endif; ?> ><?= $_title; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="col-lg-12">
                ...
                <ul class="pagination pagination-lg pull-right" style="margin: 0 0 ;">
                    <?php for ($_page = 1; $_page <= $pages['total_page']; $_page++): ?>
                        ...
                        <?php else: ?>
                            <li>
                                <a href="<?= UrlService::buildWebUrl('/account/index', ['p' => $_page, 'status' => $search_conditions['status'], 'mix_kw' => $search_conditions['mix_kw']]); ?>"><?= $_page ?></a>
                            </li>
                        <?php endif; ?>
                    <?php endfor; ?>
                </ul>
